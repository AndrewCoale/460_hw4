...
(define (change-store [new-cell : Storage] [sto : Store])           ; new function to replace override-store
...
(define (interp [a : Exp] [env : Env] [sto : Store]) : Result
  (type-case Exp a
    ...
    [(setboxE bx val) 
     (with [(v-b sto-b) (interp bx env sto)]  ; v-b: location
       (with [(v-v sto-v) (interp val env sto-b)] ; v-v: value
         (type-case Value v-b
           [(boxV l)
            (v*s v-v
